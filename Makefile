...
ttable: main.o parsers.o stack.o printers.o
	clang $(foreach req, $+, $(output_dir)/$(req)) -o $@ -Wall -Wextra -O2 -g

# Pattern rule for building object files from source
%.o : %.c ./bin
	clang $< -o $(output_dir)/$@ -c -Wall -Wextra -O2 -g

# Create a bin directory, if it doesn't exist
./bin:
	mkdir bin

# Build the memory stress tests code
run_tests: memory_tests.o stack.o
	clang $(foreach req, $+, $(output_dir)/$(req)) -o $@ -Wall -Wextra -O2 -g


# Clean ./bin (Removes EVERYTHING in bin)
clean:
	rm -f $(output_dir)/* ttable run_tests
	rmdir ./bin